...
# Title
st.markdown("<h1 style='text-align: center; text-shadow: 0px 0px 10px #00ffff;'>🚦 AI Traffic Management System</h1>",
            unsafe_allow_html=True)
    ...
    with placeholder.container():
        ...
        if traffic_data:
            ...
            with col1:
                ...
                st.markdown(f"<h2 style='text-align: center; color: #0ff;'>{traffic_data['vehicleCount']}</h2>",
                            unsafe_allow_html=True)

            with col2:
                st.markdown("<h4 style='text-align: center;'>🛣️ Congestion Level</h4>", unsafe_allow_html=True)
                st.markdown(f"<h2 style='text-align: center; color: #0ff;'>{traffic_data['congestionLevel']}</h2>",
                            unsafe_allow_html=True)

            with col3:
                st.markdown("<h4 style='text-align: center;'>⏳ Signal Time Adjusted</h4>", unsafe_allow_html=True)
                st.markdown(f"<h2 style='text-align: center; color: #0ff;'>{traffic_data['signalTime']} sec</h2>",
                            unsafe_allow_html=True)
